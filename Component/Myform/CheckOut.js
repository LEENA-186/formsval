...
let initFormState = {
  ...
  AccountType: {
    ...
    options: [
      ...
      { value: "Fixed deposit ", displayValue: "Fixed deposit" },
      { value: "Recurring deposit", displayValue: "Recurring deposit" },
      { value: "NRI account", displayValue:"NRI account" },
    ],
  },
  AadharNumber: {
    name: "AadharNumber",
...
class CheckOut extends Component {
  static contextType = data//because of class component we are giving static context type = data
  constructor(props) {
    super(props);
    this.state = {
      formIsValid: false,
  ...
  formSubmitHandler = (e) => {
    ...
    e.preventDefault();
    const context = this.context // storing that context values ie what ever present in the context file we are giving it in context  
    const formData = {};
    for (let formElementId in this.state.formControls) {
      formData[formElementId] = this.state.formControls[formElementId].value;
    }
    const push =[...context.account,formData]
    // spread operator is coping the account values as objects then adding form data values
    context.setAccount(push)
    //setting push value into setAccount
    this.setState({
      formControls: initFormState,
      formIsValid: false,
    });
  ...
  render() {
    return (
      <div className="centermine">
        ...
        />
         <Select
          title={this.state.formControls.AccountType.title}
          name={this.state.formControls.AccountType.name}
          placeholder={this.state.formControls.AccountType.placeholder}
          value={this.state.formControls.AccountType.value}
          handleChange={this.changeHandler}
          options={this.state.formControls.AccountType.options}
          touched={this.state.formControls.AccountType.touched}
          valid={this.state.formControls.AccountType.valid}
          errorMsg={this.state.formControls.AccountType.errorMsg}
        />
        <Input
          inputType={"text"}
