...
useEffect(() => {
  localStorage.setItem('history',JSON.stringify(phoneNumber));
}, [phoneNumber]);
// const names = [
//   'Ravi',
//   'Dhanush',
...
setTimeout(() => {
  setPhoneInput((prev) => {
    return { ...prev, isloading: false, show: true };
  });}, 4000);
          ...
          <Row>
            <Col>
              <form className="phoneform" onSubmit={handleSubmit}>
                ...
                {phoneInput.isloading  &&   <Spinner className='spinner' animation="grow" size="sm" />}
                <Link to="/home">
                  {' '}
                  <button
                    type="submit"   
